...
vars <- with(d,data.frame( 1/muPtGen,
                           theta_i,
#                           as.integer(( ifelse( 2*atan(exp(-muEtaGen))<3.1415927/2,
#                                     2*atan(exp(-muEtaGen)),
...
truePosPlot <- ggplot(data.frame(pT = c( binning, binning ),
                 ...
                 labs(x = expression(paste(p[T] ^{thr.}," (GeV/c)")),
                      y = "true positive"
                 ) +
...
uncompROCdf   <- data.frame( truePos  = roc[["myEff"]],
                             falsePos = roc[["myFake"]],
                             model    = rep("uncompressed",nBins)
                 )
refROCdf     <- data.frame( truePos  = roc[["refEff"]],
                            falsePos = roc[["refFake"]],
                            model    = rep("reference",nBins)
                )

compsFinalDF <- comp7ROCdf
compsFinalDF <- rbind(compsFinalDF,uncompROCdf)
compsFinalDF <- rbind(compsFinalDF,refROCdf)

compsFinalPlot <- ggplot(compsFinalDF, aes(y = truePos, x = falsePos, group = model, colour = model)) +
        geom_line() +
        geom_point(shape=1,size=0.1) +
        theme(
            title = element_text(size=15),
