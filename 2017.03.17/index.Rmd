...
## The code below crashes if ran in slidify "Error in lazyLoadDBinsertValue(data, datafile, ascii, compress, envhook)"
## It'll only run here if vars are replaced with a smaller size testSet:
...
#                )
#
## so I ran it interactively and restore the results from a file:
load("pp.RData")
...
#tt <- turnOns(pp)
#suppressWarnings( invisible( capture.output( tt[["30"]] ) ) )
```
...
effRatePlot <- ggplot(data.frame(binning,
                                 eff = c(roc$myEff, roc$refEff),
                                 rate = c(myRate, refRate),
                                 model = c(rep("myModel",nBins), rep("reference",nBins)) 
                                ),
