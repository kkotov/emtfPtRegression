...
Bottom-left part of the graphs corresponds to thresholds above ~50 GeV/$c$ $\rightarrow$ ignore it
...
barPlotScheme1 <- ggplot( data.frame( importance = modelFit1$variable.importance[c(1:4,10:12,15:25)],
                                      name       = names(modelFit1$variable.importance[c(1:4,10:12,15:25)])
                          ), aes(x=reorder(name,c(18,1:17)), y=importance)
                  ) +
...
barPlotScheme4 <- ggplot( data.frame( importance = modelFit4$variable.importance[c(1:4,7:12)],
                                      name       = names(modelFit4$variable.importance[c(1:4,7:12)])
                          ), aes(x=reorder(name,c(10,1:9)), y=importance)
                  ) +
                  geom_bar(stat="identity", col="red",fill="red") +
                  coord_flip() +
                  theme(
...
barPlotScheme2 <- ggplot( data.frame( importance = modelFit2$variable.importance[c(1:4,10:12,15:25)],
                                      name       = names(modelFit2$variable.importance[c(1:4,10:12,15:25)])
                          ), aes(x=reorder(name,c(18,1:17)), y=importance)
                  ) +
                  geom_bar(stat="identity", col="red",fill="red") +
                  coord_flip() +
...
barPlotScheme5 <- ggplot( data.frame( importance = modelFit5$variable.importance[c(1:4,7:11)],
                                      name       = names(modelFit5$variable.importance[c(1:4,7:11)])
                          ), aes(x=reorder(name,c(9,1:8)), y=importance)
                  ) +
                  geom_bar(stat="identity", col="red",fill="red") +
                  coord_flip() +
                  theme(
...
vars3p <- with(vars,data.frame( muPtGenInv,
                                ptTrg,
                           abs(msb(theta,7,0)) + ifelse(ring1==1 || ring1==4, 0, 6),
                           abs(sat(dPhi12,10)),
                           abs(sat(dPhi23,10)),
                           abs(sat(dPhi34,10)),
                           as.factor(ifelse(dPhi23*dPhi12>=0,0,1)),
...
vars5p <- with(vars,data.frame( muPtGenInv,
                           ...
                           as.factor(ifelse(dPhi23*dPhi12>=0,0,1)),
                           as.factor(ifelse(dPhi34*dPhi12>=0,0,1)),
                           abs(msb(dPhi12,10,3)) + ifelse(dPhi23*dPhi12>=0,1,-1)*abs(sat(dPhi23,5)), # analogous to dPhi13
                           abs(msb(dPhi12,10,3)) + ifelse(dPhi23*dPhi12>=0,1,-1)*abs(sat(dPhi23,5)) + ifelse(dPhi34*dPhi12>=0,1,-1)*abs(sat(dPhi34,5)),
                           abs(sat(dPhi23,5))    + ifelse(dPhi34*dPhi12>=0,1,-1)*abs(sat(dPhi34,5)), # analogous to dPhi24
                           abs(sat(dTheta14,2)),
                           c(0,0,0,0,1,1,2,2,3,3,3,0,0,0,0,0)[bitwAnd(as.integer(as.character(clct1)),0xF)+1],
                           fr1
...
vars5pp <- with(vars,data.frame( muPtGenInv,
                           ...
                           as.factor(ifelse(dPhi23*dPhi12>=0,0,1)),
                           as.factor(ifelse(dPhi34*dPhi12>=0,0,1)),
                           abs(msb(dPhi12,10,3)) + ifelse(dPhi23*dPhi12>=0,1,-1)*abs(msb(dPhi23,8,3)), # analogous to dPhi13
                           abs(msb(dPhi12,10,3)) + ifelse(dPhi23*dPhi12>=0,1,-1)*abs(msb(dPhi23,8,3)) + ifelse(dPhi34*dPhi12>=0,1,-1)*abs(msb(dPhi34,8,3)),
                           abs(msb(dPhi23,8,3))  + ifelse(dPhi34*dPhi12>=0,1,-1)*abs(msb(dPhi34,8,3)), # analogous to dPhi24
                           abs(sat(dTheta14,2)),
                           c(0,0,0,0,1,1,2,2,3,3,3,0,0,0,0,0)[bitwAnd(as.integer(as.character(clct1)),0xF)+1],
                           fr1
                             )
