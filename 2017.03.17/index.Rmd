...
colnames(vars5p) <- c("muPtGenInv", "ptTrg", predictors5 )
...
invisible( capture.output( modelFit5p <- ranger(f5, data=trainSet5p, importance="impurity") ) )
...
colnames(vars5pp) <- c("muPtGenInv", "ptTrg", predictors5 )
...
invisible( capture.output( modelFit5pp <- ranger(f5, data=trainSet5pp, importance="impurity") ) )
```
...
colnames(tszb5p) <- c("ptRef", predictors5 )
...
colnames(tszb5pp) <- c("ptRef", predictors5 )
...
## Scheme #1 $\rightarrow$ #2 transition
...
barPlotScheme2 <- ggplot( data.frame( importance = modelFit2$variable.importance[1:18],
                                      name       = names(modelFit2$variable.importance[1:18])
                          ), aes(x=name, y=importance)
                  ) +
                  geom_bar(stat="identity", col="red",fill="red") +
                  coord_flip() +
