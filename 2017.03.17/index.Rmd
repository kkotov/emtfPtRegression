...
Predictors: $\Delta\phi \times 6$, $\Delta\theta \times 6$, $CLCT \times 4$, $FR \times 4$, scaling reference thresholds by $1.4$:
...
#              binning = binning
#      )
#suppressWarnings( invisible( capture.output( tt[["30"]] + xlim(0, 100) ) ) )
```
...
vars2 <- with(vars,data.frame( muPtGenInv,
                           ...
                           abs(sat(dPhi12,15)) + ifelse(dPhi23*dPhi12>=0,1,-1)*abs(sat(dPhi23,15)), # analogous to dPhi13
                           abs(sat(dPhi12,15)) + ifelse(dPhi23*dPhi12>=0,1,-1)*abs(sat(dPhi23,15)) + ifelse(dPhi34*dPhi12>=0,1,-1)*abs(sat(dPhi34,15)),
                           abs(sat(dPhi23,15)) + ifelse(dPhi34*dPhi12>=0,1,-1)*abs(sat(dPhi34,15)), # analogous to dPhi24
                           abs(sat(dTheta12,2)),
                           abs(sat(dTheta23,2)),
                           abs(sat(dTheta34,2)),
