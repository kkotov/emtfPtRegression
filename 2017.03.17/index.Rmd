...
* "saturate" predictors at $N$ bits (e.g. $|\Delta\theta_{12}|_2\in\{0,1,2,3\}$)
...
vars1 <- with(vars,data.frame( muPtGenInv,
                           ...
                           abs(sat(dPhi12,10)) + ifelse(dPhi23*dPhi12>=0,1,-1)*abs(sat(dPhi23,10)), # analogous to dPhi13
                           abs(sat(dPhi12,10)) + ifelse(dPhi23*dPhi12>=0,1,-1)*abs(sat(dPhi23,10)) + ifelse(dPhi34*dPhi12>=0,1,-1)*abs(sat(dPhi34,10)),
                           abs(sat(dPhi23,10)) + ifelse(dPhi34*dPhi12>=0,1,-1)*abs(sat(dPhi34,10)), # analogous to dPhi24
                           abs(sat(dTheta12,5)),
                           abs(sat(dTheta23,5)),
                           abs(sat(dTheta34,5)),
                           as.factor(ifelse(dTheta23*dTheta12>=0,0,1)),
                           as.factor(ifelse(dTheta34*dTheta12>=0,0,1)),
                           abs(sat(dTheta12,5)) + ifelse(dTheta23*dTheta12>=0,1,-1)*abs(sat(dTheta23,5)), # analogous to dTheta13
                           abs(sat(dTheta12,5)) + ifelse(dTheta23*dTheta12>=0,1,-1)*abs(sat(dTheta23,5)) + ifelse(dTheta34*dTheta12>=0,1,-1)*abs(sat(dTheta34,5)),
                           abs(sat(dTheta23,5)) + ifelse(dTheta34*dTheta12>=0,1,-1)*abs(sat(dTheta34,5)), # analogous to dTheta24
                           clct1,
                           clct2,
                           clct3,
                           clct4,
...
vars2 <- with(vars,data.frame( muPtGenInv,
                           ...
                           as.factor(ifelse(dPhi23*dPhi12>=0,0,1)),
                           as.factor(ifelse(dPhi34*dPhi12>=0,0,1)),
                           abs(sat(dPhi12,7)) + ifelse(dPhi23*dPhi12>=0,1,-1)*abs(sat(dPhi23,5)), # analogous to dPhi13
                           abs(sat(dPhi12,7)) + ifelse(dPhi23*dPhi12>=0,1,-1)*abs(sat(dPhi23,5)) + ifelse(dPhi34*dPhi12>=0,1,-1)*abs(sat(dPhi34,5)),
                           abs(sat(dPhi23,7)) + ifelse(dPhi34*dPhi12>=0,1,-1)*abs(sat(dPhi34,5)), # analogous to dPhi24
                           abs(sat(dTheta12,5)),
                           abs(sat(dTheta23,5)),
                           abs(sat(dTheta34,5)),
                           as.factor(ifelse(dTheta23*dTheta12>=0,0,1)),
...
## Tuning compression further

type | $\theta$ | $\Delta\phi_{12}$ | $\Delta\phi_{23}$ | $\Delta\phi_{34}$ | $S^\phi_{12,23}$ | $S^\phi_{12,34}$ | $\Delta\theta_{12}$ | $\Delta\theta_{23}$ | $\Delta\theta_{34}$ | $\Delta\theta_{13}$ | $\Delta\theta_{14}$ | $\Delta\theta_{24}$ | clct | fr
-----|----------|-------------------|-------------------|-------------------|------------------|------------------|---------------------|---------------------|---------------------|---------------------|---------------------|---------------------|---------------|-----------
#1   |  10      |      10           |        10         |         10        |         1        |        1         |         5           |         5            |         5          |         5           |       5             |           5         |  4,4,4,4 | 1,1,1,1 
#4'  | `5`      |      10           |       `5`         |        `5`        |         1        |        1         |        `0`          |        `0`           |        `0`         |         `0`         |         `3`         |         `0`         |  4,`0,0,0` | 1,`0,0,0` 
#5'  |  5       |      10           |       `4`         |        `4`        |         1        |        1         |         0           |         0            |         0          |          0          |         `2`         |          0          |  `2`,0,0,0 | 1,0,0,0 
#5'' | `4`      |      10           |       `4`         |        `4`        |         1        |        1         |         0           |         0            |         0          |          0          |         `2`         |          0          |  `2`,0,0,0 | 1,0,0,0 


```{r message = F, echo = F}
# type #4'

vars4p <- with(vars,data.frame( muPtGenInv,
                                ptTrg,
                           abs(sat(theta,5)),
                           abs(sat(dPhi12,10)),
                           abs(sat(dPhi23,5)),
                           ...
                           as.factor(ifelse(dPhi23*dPhi12>=0,0,1)),
                           as.factor(ifelse(dPhi34*dPhi12>=0,0,1)),
                           abs(sat(dPhi12,10)) + ifelse(dPhi23*dPhi12>=0,1,-1)*abs(sat(dPhi23,5)), # analogous to dPhi13
                           abs(sat(dPhi12,10)) + ifelse(dPhi23*dPhi12>=0,1,-1)*abs(sat(dPhi23,5)) + ifelse(dPhi34*dPhi12>=0,1,-1)*abs(sat(dPhi34,5)),
                           abs(sat(dPhi23,10)) + ifelse(dPhi34*dPhi12>=0,1,-1)*abs(sat(dPhi34,5)), # analogous to dPhi24
                           abs(sat(dTheta14,3)),
                           clct1,
                           fr1
                             )
...
colnames(vars4p) <- c("muPtGenInv", "ptTrg", predictors4 )

set.seed(1)

trainSet4p <- vars4p[part,]
testSet4p <- vars4p[-part,]

invisible( capture.output( modelFit4p <- ranger(f4, data=trainSet4p, importance="impurity") ) )
```
...
vars5p <- with(vars,data.frame( muPtGenInv,
                           ...
                           as.factor(ifelse(dPhi23*dPhi12>=0,0,1)),
                           as.factor(ifelse(dPhi34*dPhi12>=0,0,1)),
                           abs(sat(dPhi12,10)) + ifelse(dPhi23*dPhi12>=0,1,-1)*abs(sat(dPhi23,4)), # analogous to dPhi13
                           abs(sat(dPhi12,10)) + ifelse(dPhi23*dPhi12>=0,1,-1)*abs(sat(dPhi23,4)) + ifelse(dPhi34*dPhi12>=0,1,-1)*abs(sat(dPhi34,4)),
                           abs(sat(dPhi23,10)) + ifelse(dPhi34*dPhi12>=0,1,-1)*abs(sat(dPhi34,4)), # analogous to dPhi24
                           abs(sat(dTheta14,2)),
                           c(0,0,0,0,1,1,2,2,3,3,3,0,0,0,0,0)[bitwAnd(as.integer(as.character(clct1)),0xF)+1],
                           fr1
                             )
...
tszb4p <- with(tszb,data.frame( ptRef,
                           abs(sat(theta,5)),
                           abs(sat(dPhi12,10)),
                           abs(sat(dPhi23,5)),
                           abs(sat(dPhi34,5)),
...
colnames(tszb4p) <- c("ptRef", predictors4 )

invisible( capture.output( tszb4p$res <- 1/predict(modelFit4p,tszb4p)$predictions ) )

rate4p  <- sapply(binning, function(x) sum(tszb4p$res>x) )
...
```{r message = F, echo = F}
invisible( capture.output(
pp4p <- preprocess( modelFit = modelFit4p,
                    testSet  = testSet4p,
                    rateShapeBinned,
                    binning
                  )
) )
myTurnOnComp4p <- pp4p$getMyTurnOn()
rm(pp4p)
```
...
```{r, fig.height=3., fig.width=4., fig.align = 'center', comment = NA, message = F, echo = F, dev='svg'}

roc4p <- rocMetric( rateShapeBinned = rateShapeBinned,
                    myModelTurnOn = myTurnOnComp4p,
                    referenceTurnOn = refTurnOn,
                    binning = binning,
                    refScale = 1.43
         )
...
refModelROCdf <- data.frame( truePos  = roc4p[["refEff"]],
                             falsePos = roc4p[["refFake"]],
                             model    = rep("baseline",nBins)
                 )
myModelROCdf <- data.frame( truePos  = roc5p[["refEff"]],
                            falsePos = roc5p[["refFake"]],
                            model    = rep("scheme #1",nBins)
                )
comp4pROCdf   <- data.frame( truePos  = roc4p[["myEff"]],
                             falsePos = roc4p[["myFake"]],
                             model    = rep("scheme #4'",nBins)
                 )
...
comp4pROCdf     <- comp4pROCdf  [start:nBins,]
comp5pROCdf     <- comp5pROCdf  [start:nBins,]
comp5ppROCdf    <- comp5ppROCdf [start:nBins,]

compspDF <- rbind(myModelROCdf,refModelROCdf)
compspDF <- rbind(compspDF,comp4pROCdf)
...
compspPlot <- ggplot(compspDF, aes(y = truePos, x = falsePos, group = model, colour = model)) +
        ...
        scale_colour_manual(values = c("baseline"     = "#F8766D",
                                       "scheme #1"    = "#7CAE00",
                                       "scheme #4'"   = "#CD9600",
                                       "scheme #5'"   = "#00BE67",
                                       "scheme #5''"  = "#00A9FF"
                                     ),
                            breaks = c("baseline", "scheme #1", "scheme #4'", "scheme #5'", "scheme #5''"),
                            labels = c("baseline", "scheme #1", "scheme #4'", "scheme #5'", "scheme #5''")
        ) +
...
effRateCompspDF <- rbind( data.frame( eff   = roc1[["myEff"]], #roc1[["refEff"]], #uncompressed
                                      rate  = rate1, #myRate,
                                      model = rep("scheme #1",nBins)
                          ),
                          effRateCompspDF
                   )
effRateCompspDF <- rbind( data.frame( eff   = roc4p[["myEff"]],
                                     rate  = rate4p,
                                     model = rep("scheme #4'",nBins)
                          ),
                          effRateCompspDF
...
Schemes #1, #4' (compress $\theta$, $\Delta\phi_{23,34}$, $\Delta\theta$), #5' ($\Delta\phi_{23,34}$, $clct1$, $\Delta\theta$) and #5'' ($\theta$) are close


--- &twocol

## Observations on the tuned schemes


Relative importance of predictors in Scheme #1 (left) and #5'' (right):

*** =left

```{r, fig.height=3., fig.width=4.5, fig.align = 'center', comment = NA, message = F, echo = F, dev='svg'}

#barplot( modelFit1$variable.importance[1:18], horiz=T, col='red', xlab='Delta MSE', las=2, main='Scheme #1' )

barPlotScheme1 <- ggplot( data.frame( importance = modelFit1$variable.importance[1:18],
                                      name       = names(modelFit1$variable.importance[1:18])
                          ), aes(x=name, y=importance)
                  ) +
...
#barplot( modelFit2$variable.importance[1:18], horiz=T, col='red', xlab='Delta MSE', las=2, main='Scheme #2' )

barPlotScheme2 <- ggplot( data.frame( importance = modelFit5pp$variable.importance,
                                      name       = names(modelFit5pp$variable.importance)
                          ), aes(x=name, y=importance)
                  ) +
                  geom_bar(stat="identity", col="red",fill="red") +
                  coord_flip() +
