...
#    sum( (rate - estRate)^2 / estRate ) # gaus works if all estRate >~5
#    sum( ifelse(rate, rate*(log( estRate/rate ) + 1), 0) ) # poisson likelihood
...
qwe I'll stick with this parameterization keeping in mind the excluded [1-2] GeV/$c$ bin

--- .class #id

## Deconvoluting underlying rate

Construct likelihood $\sim \prod_i Poiss(r_i | e_i ^j R_j)$ where, as [before](https://kkotov.github.io/emtfPtRegression/2017.02.10/#8),
* $r_i$ - measured rate above threshold $i$
* $R_j$ - true (differential) underlying rate in bin $j$ of true $p_T$
* $e_i ^j$ - turn-on efficieny in $i^{th}$ bin of true $p_T$ for threshold $j$
...
#    sum( ifelse(rate, rate*( log( ((t(turnOn) %*% estRate) + rate[nBins])/rate ) + 1), 0) ) # poisson
...
Using parametric form for $R$ and turn-ons $e$ from the [last talk](https://kkotov.github.io/emtfPtRegression/2017.02.10/#5) I can assess $R$ from $r$
