...
binLikelihood <- function(par, rate) {
    norm1     <- par[1]
    powerLaw1 <- par[2]
    norm2     <- par[3]
    powerLaw2 <- par[4]
...
#suppressWarnings( fitMes <- nlm(binLikelihood, c(1,2,1,1,10), rate = rateThr, hessian=T) )
suppressWarnings( fitMes <- nlm(binLikelihood, c(1.8,1.2,17,3.1,3), rate = rateBin, hessian=T) )
#solve(fitMes$hessian) # to get covariance matrix if needed

norm1     <- fitMes$estimate[1]
powerLaw1 <- fitMes$estimate[2]
norm2     <- fitMes$estimate[3]
powerLaw2 <- fitMes$estimate[4]
shift     <- fitMes$estimate[5]
approxRate <- sapply(binning[1:(length(binning)-1)],function(x) rateBin[1]*(norm1/exp(x*powerLaw1) + norm2/((x+shift)^powerLaw2)))
#approxRate <- sapply(binning,function(x) rateThr[1]*(norm1/exp(x*powerLaw1) + norm2/((x+shift)^powerLaw2)))

#twoComps <- function(x,norm1,powerLaw1,norm2,powerLaw2,shift) norm1/exp(x*powerLaw1) + norm2/((x+shift)^powerLaw2)
#MASS::fitdistr(ptMes, twoComps, list(norm1 = 3, powerLaw1 = 0.6, norm2 = 1, powerLaw2 = 1, shift = 10))$estimate
```
...
rrPlot <- ggplot() +
        geom_line( data = data.frame(pT = binning[1:(length(binning)-1)], cnt = rateBin) ,
                        aes( x = pT,
                             y = cnt,
                             colour = "r",
                             fill = "r"
        ...
        geom_line( data = data.frame(pT = binning[1:(length(binning)-1)], cnt = approxRate),
                        aes( x = pT,
                             y = cnt,
                             colour = "b",
                             fill = "b"
...
The $1/p_T^{2.4}$, although, not ideal, seems to be a good 
