...
pt0.33 <- runif(30000000,0.000001,1)^(-1/2)
rate0.33 <- sapply(binning[2:nBins], function(x) sum(pt3 > x) )

pt0.25 <- runif(30000000,0.00001,1)^(-1/3)
rate4 <- sapply(binning[2:nBins], function(x) sum(pt4 > x) )

pt0.45 <- runif(30000000,0.00001,1)^(-0.45)
rate045 <- sapply(binning[2:nBins], function(x) sum(pt4 > x) )
...
```{r, fig.height=5., fig.width=5., fig.align = 'center', comment = NA, message = F, echo = F}
ggplot( data.frame(qqplot(x=pt0.45, y=ptMes, plot.it=FALSE)) ) + geom_point(aes(x=x, y=y))
```
