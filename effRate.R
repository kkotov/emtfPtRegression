                   ...
                   )
{
    ...
    # efficiency part: events with the responce_pT over the threshold (given by every bin) count towards efficiency
    # the matrices below are indexed by [true_pT_bin,pT_bin] and represent number of events with the responce_pT > pT_bin
    ...
    #  1. minimising efficiency[ for all true_pT_bin(s), pT_bin < true_pT_bin ] (false-positives %)
    #  2. maximizing efficiency[ for all true_pT_bin(s), pT_bin > true_pT_bin ] (true-positives %)
    ...
    #  have the same cost: accidently promoting true_pT of 5 GeV to 30 GeV (above some threshold)
    ...
    # rate part: to have a realistic trigger counts I convolute the rateShape with turn-on curve
    # create a simple histogram of the rateShape
    rateShapeBinned <- sapply( by(rateShape$trigRate, sapply(rateShape$true_pT, findBin, binning), sum), function(x) x )
    # rates are functions of pT_bin (threshold) only, convolute rateShape with efficiencies over true_pT_bin
    myModelRate   <- drop(rateShapeBinned %*% (myModelCount / pSpec$count))
    referenceRate <- drop(rateShapeBinned %*% (referenceCount / pSpec$count))

    # finally, put efficiencies and rates together on one ROC-like plot
    #  the pT_bin (threshold) is a "natural parameter" on the efficiency-rate curve
    nBins <- length(binning)
    effRate <- data.frame( pT_bin = binning,
                           rate = myModelRate,
                           eff  = sapply(1:nBins,function(x) sum(myModelCount[x:nBins,x])/sum(pSpec$count[x:nBins]))
                         )

model = factor(rep("myModel",length(binning)),rep("reference",length(binning))),

    ggplot(effRate, aes(x = eff, y = rate, group = model, colour = model)) + 
