...
The metrices above will always favor more complex models fitting in-sample noise
...
Lasso calculates model parameters $\beta$ by minimizing $RSS + \lambda\Sigma_{i=1}^p|\beta_i|$
...
Forward stepwise regression "switches in" (shades of gray) predictors as follows:
...
The most important preditors seems to be dPhi and clct1 (!)
...
Out-of-sample persormance 

Cross-validation vs. analitical test error estimate

--- .class #id

## Performace metric for trees-based models
...
choise of the : 1/pT to rank up low-pT assignment
?loss function?
...
responce 1/pT (sagitta) is proportional to dPhi, let's assume linearity

* regmodels

* anova

RF: randomForest::rfcv

importance

http://stats.stackexchange.com/questions/12605/measures-of-variable-importance-in-random-forests

http://stats.stackexchange.com/questions/112556/random-forest-need-help-understanding-the-rfcv-function
